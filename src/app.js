...
function main() {
  [snake_position, is_playing] = checkIsGameOver(snake_position, direction);
  if (is_food) {
    [snake_position, is_food, food_location] = eatFood(
      snake_position[0],
      snake_position,
      is_food,
      food_location
  ...
  [next_head_position, snake_position] = calculateSnakePosition(
    snake_position,
    next_head_position
  );
  draw(snake_position);
  if (is_playing) {
    ...
    [snake_position, direction, next_head_position, is_food] = restartGame(
      food_location,
      snake_position,
      direction,
      next_head_position,
